...
extern const struct rtl83xx_reg rtl838x_reg;
extern const struct rtl83xx_reg rtl839x_reg;
 ...
 */
static void rtl83xx_rb_cleanup(struct rtl83xx_eth_priv *priv)
{
	int r;
	u32	*last;
	struct p_hdr *h;
...
static irqreturn_t rtl83xx_net_irq(int irq, void *dev_id)
{
	...
	struct rtl83xx_eth_priv *priv = netdev_priv(dev);
...
static void rtl83xx_hw_reset(struct rtl83xx_eth_priv *priv)
{
	u32 int_saved;

	printk("RESETTING %x, CPU_PORT %d\n", soc_info.family, priv->cpu_port);
	/* Stop TX/RX */
...
static void rtl83xx_hw_ring_setup(struct rtl83xx_eth_priv *priv)
{
	int i;
	struct ring_b *ring = priv->membase;
...
static void rtl83xx_hw_en_rxtx(struct rtl83xx_eth_priv *priv)
{
	u32 v;

	printk("rtl83xx_hw_en_rxtx\n");
...
static void rtl83xx_setup_ring_buffer(struct ring_b *ring)
{
	int i, j;
...
{
	unsigned long flags;
	struct rtl83xx_eth_priv *priv = netdev_priv(ndev);
	struct ring_b *ring = priv->membase;
	int err;

	printk("rtl83xx_eth_open called %x, ring %x\n", (uint32_t)priv, (uint32_t)ring);
	spin_lock_irqsave(&priv->lock, flags);
	rtl83xx_hw_reset(priv);
	rtl83xx_setup_ring_buffer(ring);
	rtl83xx_hw_ring_setup(priv);
	err = request_irq(ndev->irq, rtl83xx_net_irq, IRQF_SHARED,
			ndev->name, ndev);
	if (err) {
...
static void rtl83xx_hw_stop(struct rtl83xx_eth_priv *priv)
{
	int i;
...
static int rtl83xx_eth_stop(struct net_device *ndev)
{
	...
	printk("in rtl83xx_eth_stop %x\n", (uint32_t)priv);

	spin_lock_irqsave(&priv->lock, flags);
	phylink_stop(priv->phylink);
	rtl83xx_hw_stop(priv);
...
static void rtl83xx_eth_set_multicast_list(struct net_device *dev)
{
	//struct rtl83xx_eth_priv *priv = netdev_priv(dev);
...
static void rtl83xx_eth_tx_timeout(struct net_device *ndev, unsigned int txqueue)
{
	unsigned long flags;
	struct rtl83xx_eth_priv *priv = netdev_priv(ndev);
	printk("in rtl83xx_eth_tx_timeout %x\n", (uint32_t)priv);
	spin_lock_irqsave(&priv->lock, flags);
	rtl83xx_hw_stop(priv);
	rtl83xx_hw_ring_setup(priv);
	rtl83xx_hw_en_rxtx(priv);
...
{
	struct rtl83xx_eth_priv *priv = container_of(napi, struct rtl83xx_eth_priv, napi);
	int work_done = 0, r = 0;

	/* printk("in rtl83xx_poll_rx, budget: %d\n", budget); */
	while (work_done < budget && r < RXRINGS) {
		work_done += rtl83xx_hw_receive(priv->netdev, r, budget - work_done);
		r++;
	}
...
static void rtl83xx_validate(struct phylink_config *config,
			 unsigned long *supported,
			 struct phylink_link_state *state)
{
	__ETHTOOL_DECLARE_LINK_MODE_MASK(mask) = { 0, };

	printk("In rtl83xx_validate\n");
...
static void rtl83xx_mac_config(struct phylink_config *config,
			       unsigned int mode,
			       const struct phylink_link_state *state)
{
	...
	printk("In rtl83xx_mac_config, mode %x\n", mode);
	return;
}
...
static void rtl83xx_mac_an_restart(struct phylink_config *config)
{
	...
	printk("In rtl83xx_mac_an_restart\n");
...
static void rtl83xx_mac_pcs_get_state(struct phylink_config *config,
				  struct phylink_link_state *state)
{
	u32 speed;
	struct net_device *dev = container_of(config->dev, struct net_device, dev);
	...
	printk("In rtl83xx_mac_pcs_get_state\n");
	  // TODO: FIX CALLS
...
static void dump_mac_conf(struct rtl83xx_eth_priv *priv)
{
	int p;
...
static void rtl83xx_mac_link_down(struct phylink_config *config,
				  unsigned int mode,
				  phy_interface_t interface)
{
	...
	printk("In rtl83xx_mac_link_down\n");
	/* Stop TX/RX to port */
...
static void rtl83xx_set_mac_hw(struct net_device *dev, u8 *mac)
{
	struct rtl83xx_eth_priv *priv = netdev_priv(dev);
	unsigned long flags;

	spin_lock_irqsave(&priv->lock, flags);
	printk("In rtl83xx_set_mac_hw\n");
...
static int rtl83xx_set_mac_address(struct net_device *dev, void *p)
{
	struct rtl83xx_eth_priv *priv = netdev_priv(dev);
...
{
	struct rtl83xx_eth_priv *priv = netdev_priv(ndev);
	printk("rtl83xx_get_link_ksettings called\n");
...
{
	struct rtl83xx_eth_priv *priv = netdev_priv(ndev);
	printk("rtl83xx_set_link_ksettings called\n");

	return phylink_ethtool_ksettings_set(priv->phylink, kset);
}

static int rtl83xx_mdio_init(struct rtl83xx_eth_priv *priv)
{
	...
	if (!mii_np) {
		dev_err(&priv->pdev->dev, "no mdio-bus child node found");
		return -ENODEV;
	}
...
static int rtl83xx_mdio_remove(struct rtl83xx_eth_priv *priv)
{
	printk("rtl83xx_mdio_remove called\n");
	if (!priv->mii_bus)
		return 0;
...
static int rtl83xx_eth_probe(struct platform_device *pdev)
{
	struct net_device *dev;
	struct device_node *dn = pdev->dev.of_node;
	struct rtl83xx_eth_priv *priv;
	...
	pr_info("Probing RTL83xx eth device pdev: %x, dev: %x\n",
	...
	dev = alloc_etherdev(sizeof(struct rtl83xx_eth_priv));
	if (!dev) {
		err = -ENOMEM;
		goto err_free;
	}
	...
	dev->ethtool_ops = &rtl83xx_ethtool_ops;
	...
	if (!is_valid_ether_addr(dev->dev_addr)) {
		...
		if (rtl83xx_set_mac_address(dev, &sa))
			netdev_warn(dev, "Failed to set MAC address.\n");
	}
	pr_info("Using MAC %08x%08x\n", sw_r32(priv->r->mac),
					sw_r32(priv->r->mac + 4));
	strcpy(dev->name, "eth%d");
	dev->netdev_ops = &rtl83xx_eth_netdev_ops;
	priv->pdev = pdev;
	priv->netdev = dev;

	err = rtl83xx_mdio_init(priv);
	if (err)
		goto err_free;
	...
	netif_napi_add(dev, &priv->napi, rtl83xx_poll_rx, 64);
...
static int rtl83xx_eth_remove(struct platform_device *pdev)
{
	...
	struct rtl83xx_eth_priv *priv = netdev_priv(dev);
	if (dev) {
		printk("Removing platform driver for rtl83xx-eth\n");
		rtl83xx_mdio_remove(priv);
		rtl83xx_hw_stop(priv);
